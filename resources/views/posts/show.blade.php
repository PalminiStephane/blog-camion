                ...
                <article class="flex flex-col lg:flex-row pb-10 md:pb-16 border-b">
                    ...
                    <div class="flex flex-col items-start mt-5 space-y-5 lg:w-7/12 lg:mt-0 lg:ml-12">
                        ...
                        <p class="text-xl lg:text-2xl text-slate-600">
                            {{-- La fonction nl2br()our gére les balises br de retour a la ligne (pour l'utiliser on se sert des simple accolade) . les simple accolades ne nous protègent plus de la faille xss, pour palier a ceci on utilise la fonction htmlspecialchars: e()--}}
                            {!! nl2br(e($post->content)) !!}
                        </p>
                        <time class="text-xs text-slate-400" datetime="{{ $post->created_at }}">{{ $post->created_at->format('d/m/Y') }}</time>
                    </div>
                </article>
                {{-- Fin du post --}}
            </div>
